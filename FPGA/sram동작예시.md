...
- tRC (Read Cycle Time)
    ...
    - **DS1230W-100 값**: 100ns
    - **설명**: 메모리는 읽기 동작이 끝난 뒤, 다음 읽기 동작이 들어오기까지 최소 `100ns` 이상의 간격이 있어야 정상적인 데이터 출력이 가능함. 이보다 빠르게 반복해서 읽으면 메모리가 안정적으로 데이터 값을 내지 못함.
    - **중요**: 이론적 최대 read 반복 속도: `1 / 100ns = 10MHz`. FPGA는 더 높은 주파수로 동작해도 상관없지만 read FSM(상태머신) 설계가 read 1회당 100ns 이상을 주도록 구성되어야 함.
...
- tWC (Write Cycle Time)
    ...
    - **DS1230W-100 값:** 100ns
    - **설명:** 메모리는 쓰기 동작을 처리하는 데 최소 100ns가 필요하며, 이보다 빠르게 연속해서 쓰기 요청이 들어오면 데이터가 올바르게 기록되지 않을 수 있음. 즉, 메모리 입장에서 write cycle 간격이 ≥ 100ns 확보되어야 한다.
    - **중요:** FPGA 클럭 주파수가 얼마이든 상관없이 쓰기 FSM(상태머신)이 write 1회당 100ns 이상을 확보하도록 구성해야 함. 이를 위해 보통 쓰기 과정을 여러 클럭에 나누어 수행(Wait States)하여 메모리에 충분한 시간을 줌.
